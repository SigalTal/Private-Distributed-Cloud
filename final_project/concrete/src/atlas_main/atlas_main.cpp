...
int NbdHandle(int* socfd)
{
    ...
    AtlasHeader_t atlas_header = ConvertToAtlas(req);
    struct msghdr msg = {};
    struct iovec payload[2]={0};

    payload[0].iov_base = &atlas_header;
    payload[0].iov_len = sizeof(AtlasHeader_t);
    msg.msg_iovlen = 1;

    if(req->reqType == NBD_CMD_WRITE)
    {
        payload[1].iov_base = req->dataBuf;
        payload[1].iov_len = req->dataLen;
        msg.msg_iovlen = 2;
    }

    msg.msg_iov = payload;
    sendmsg(socfd[1],&msg,0);
